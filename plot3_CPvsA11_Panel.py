...
def plotter_3_CPvsA11_Panel(input_dir, # Format_1 requires input_dir
    ...
    """
    Generates specific panel plots comparing Cellpose results with A11 simulation data.

    This function loads Cellpose extraction data (from `CP_extract_1`) and A11
    simulation data. Based on the boolean flags `Panel_1` to `Panel_4`, it generates
    different multi-panel figures comparing various metrics over time.

    Panel 1 (if Panel_1=1): Similar to CP_plotter_2, shows time evolution of
        diameters, radii, iHRR, cell count, and CP efficiency.
    Panel 2 (if Panel_2=1): Focuses on non-dimensionalization checks, plotting
        dimensional and non-dimensional radii and diameters.
    Panel 3 (if Panel_3=1): Replicates Panel 1 layout but potentially for different data or settings.
    Panel 4 (if Panel_4=1): Creates a 5x2 panel figure comparing various A11 metrics
        (left column) against Cellpose metrics (right column, plotted on twin axes).
    ...
    CP_data_df : pandas.DataFrame or None, optional
        A pre-loaded DataFrame containing the segmentation data. Overrides loading
        from the `.pkl` file if provided. Defaults to None.
    ...
    Panel_2 : int, optional
        If 1, generates and shows 'plot_panel_2.png' (non-dimensionalization check). Defaults to 0.
    ...
    - Panel plots are saved with specific filenames (e.g., `plot_panel_4.png`).
    - The `video` parameter is not currently implemented for this plotter.
    """
    #################################################### I/O
    #
    ...
    if Panel_1 == 1:
        ...
        S2 = 1e-1
        ax_1_12.plot(CP_data_df['time'], CP_data_df['R_SF_nonDim'] * S2, label=f"Image deduced Spherical Flame Radius * {S2:.3f}", color='olive')
        ...
        ax_1_12_R.plot(CP_data_df['time'], CP_data_df['N_cells'], label=f"Number of cells", color='red')
        
        # Create a third y-axis for the dotted line plots
        ax_1_12_RR = ax_1_12.twinx()  # Second twin axis
        ax_1_12_RR.plot(CP_data_df['time'], CP_data_df['Ar_px2_CP_maskperSF'], label="$A_{Cell masks}/A_{Spherical Flame}$", color='gray')
    ...
    if Panel_2 == 1:
        ...
        # First plot: Plotting data from CP_data_df
        
        # Plotting the first set of data
        ax_1.plot(CP_data_df['time'], CP_data_df['R_SF_nonDim'], label="R_SF_nonDim", color='olive', linestyle='solid')
        ...
        ax_1_twin2.plot(CP_data_df['time'], CP_data_df['D_SF_px'], label="D_SF_px", color='blue', linestyle='dashed')
        ...
        CP_data_df['R_mean_interpolated_i'] = (CP_data_df['nonDim_per_px'] * CP_data_df['D_SF_px']) / 2
        ax_1_twin3.plot(CP_data_df['time'], CP_data_df['R_mean_interpolated_i'], label="R_mean_interpolated_i", color='black', linestyle='solid')
        ...
        ax_2.plot(CP_data_df['time'], CP_data_df['nonDim_per_px'], label="nonDim_per_px", color='blue', linestyle='solid')
        ...
        ax_3_twin.plot(CP_data_df['time'], CP_data_df['diameter_mean_px'], label="diameter_mean_px", color='green', linestyle='dashed')
    ...
    if Panel_3 == 1:
        ...
        S2 = 1e-1
        ax_0.plot(CP_data_df['time'], CP_data_df['R_SF_nonDim'] * S2, label=f"Image deduced Spherical Flame Radius * {S2:.3f}", color='olive')
        ...
        ax_0_R = ax_0.twinx()
        ax_0_R.plot(CP_data_df['time'], CP_data_df['N_cells'], label=f"Number of cells", color='red')
        
        # Create a third y-axis for the dotted line plots
        ax_0_RR = ax_0.twinx()  # Second twin axis
        ax_0_RR.plot(CP_data_df['time'], CP_data_df['Ar_px2_CP_maskperSF'], label="$A_{Cell masks}/A_{Spherical Flame}$", color='gray')
        ...
        ax_0_R.set_ylim(CP_data_df['N_cells'].min(), CP_data_df['N_cells'].max()*1.05)
    ...
    if Panel_4 == 1:
        ...
        for ax in axes_R:
            ...
            ax_R2 = ax.twinx()
            ax_R2.plot(CP_data_df['time'], CP_data_df['N_cells'],
                        label="CP Number of cells $N_c$", color='red')
            ax_R2.set_ylabel("CP Number of cells $N_c$", color='red')

            ax_R3 = ax.twinx()
            ax_R3.plot(CP_data_df['time'], CP_data_df['Ar_px2_CP_maskperSF'],
            ...
            ax_R2.set_ylim(CP_data_df['N_cells'].min(), CP_data_df['N_cells'].max() * 1.05)
            ax_R3.set_ylim(0, 1)
