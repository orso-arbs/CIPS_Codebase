...
def plotter_14_xTwinyyyy_image_histogram(
    input_dir,
    ...
    image_width_ratio=0.2,     # Width ratio for image subplot
    plot_width_ratio=0.4,      # Width ratio for property plot subplot
    hist_width_ratio=0.4,      # Width ratio for histogram subplot
    plot_spacing=0.15,          # Horizontal spacing between plots (legacy parameter, kept for compatibility)
    image_plot_spacing=0.05,    # Horizontal spacing between image and plot
    plot_hist_spacing=0.25,     # Horizontal spacing between plot and histogram
    ...
    dist_column='d_cell_SRec_distribution_nonDim',  # Column name for the distribution to plot
    dist_color='green',                             # Color for the distribution
    dist_label='Cell Diameter',                     # Label for the distribution
    hist_x_label='Cell Diameter (Non-Dimensional)', # X-axis label for histogram
    hist_legend=True,
    hist_title_template='',                        # Title template for histogram
    bin_count=50,                                   # Number of histogram bins
    hist_alpha=0.6,                                 # Transparency of histogram bars
    show_mean_line=True,                            # Whether to show mean line
    mean_line_color='darkgreen',                          # Color for mean line
    mean_line_style='--',                           # Line style for mean line
    mean_line_width=2,                              # Line width for mean line
    mean_line_label_template=r'$\overline{d}_c$: {:.3f}',     # Template for mean line label with LaTeX formatting
    hist_x_axis_limit=None,                         # Manual x-axis limit for histogram
    hist_y_axis_limit=None,                         # Manual y-axis limit for histogram
    hist_x_axis_extension=1.1,                      # Factor to extend hist x-axis
    hist_y_axis_extension=1.1,                      # Factor to extend hist y-axis
    hist_normalize_percentage=True,                 # Whether to normalize histogram to percentage
    hist_show_grid=True,                            # Whether to show grid on histogram
    hist_grid_style='--',                           # Style of histogram grid lines
    hist_grid_width=0.5,                            # Width of histogram grid lines
    hist_grid_color='gray',                         # Color of histogram grid lines
    hist_grid_alpha=0.3,                            # Alpha of histogram grid lines
):
    """
    Creates an x-y plot with multiple twin y-axes alongside the current image and a histogram, each with configurable parameters.
    ...
    image_width_ratio : float, optional
        Width ratio for image subplot, by default 0.2
    plot_width_ratio : float, optional
        Width ratio for property plot subplot, by default 0.4
    hist_width_ratio : float, optional
        Width ratio for histogram subplot, by default 0.4
    plot_spacing : float, optional
        Horizontal spacing between plots, by default 0.15
    image_plot_spacing : float, optional
        Horizontal spacing between image and plot, by default 0.05
    plot_hist_spacing : float, optional
        Horizontal spacing between plot and histogram, by default 0.25
    ...
    include_A11_data : bool, optional
        Whether to include A11 simulation data in the plot, by default True
    ...
    dist_column : str, optional
        Column name for the distribution to plot in histogram, by default 'd_cell_SRec_distribution_nonDim'
    dist_color : str, optional
        Color for the distribution in histogram, by default 'green'
    dist_label : str, optional
        Label for the distribution in histogram, by default 'Cell Diameter'
    hist_x_label : str, optional
        X-axis label for histogram, by default 'Cell Diameter (Non-Dimensional)'
    hist_title_template : str, optional
        Title template for histogram, by default ''
    bin_count : int, optional
        Number of histogram bins, by default 50
    hist_alpha : float, optional
        Transparency of histogram bars, by default 0.6
    show_mean_line : bool, optional
        Whether to show mean line in histogram, by default True
    mean_line_color : str, optional
        Color for mean line in histogram, by default 'red'
    mean_line_style : str, optional
        Line style for mean line in histogram, by default '--'
    mean_line_width : int, optional
        Line width for mean line in histogram, by default 2
    mean_line_label_template : str, optional
        Template for mean line label in histogram, by default 'Mean {}: {:.3f}'
    hist_x_axis_limit : tuple or float, optional
        Manual x-axis limit for histogram, by default None
    hist_y_axis_limit : tuple or float, optional
        Manual y-axis limit for histogram, by default None
    hist_x_axis_extension : float, optional
        Factor to extend histogram x-axis, by default 1.1
    hist_y_axis_extension : float, optional
        Factor to extend histogram y-axis, by default 1.1
    hist_normalize_percentage : bool, optional
        Whether to normalize histogram to percentage, by default True
    hist_show_grid : bool, optional
        Whether to show grid on histogram, by default True
    hist_grid_style : str, optional
        Style of histogram grid lines, by default '--'
    hist_grid_width : float, optional
        Width of histogram grid lines, by default 0.5
    hist_grid_color : str, optional
        Color of histogram grid lines, by default 'gray'
    hist_grid_alpha : float, optional
        Alpha of histogram grid lines, by default 0.3
    ...
    if Plot_log_level >= 1:
        print(f"plotter_14_xTwinyyyy_image_histogram: Output directory: {output_dir}")
    ...
    # Calculate global histogram statistics if needed
    if dist_column in df.columns and any('distribution' in dist_column for dist_column in df.columns):
        # Clean and collect all distribution data for global statistics
        all_dist_data = []
        
        for i, row in df.iterrows():
            if dist_column in row and row[dist_column] is not None:
                dist_data = row[dist_column]
                if isinstance(dist_data, np.ndarray):
                    # Remove NaN values
                    clean_data = dist_data[~np.isnan(dist_data)]
                    if len(clean_data) > 0:
                        all_dist_data.append(clean_data)
        
        # Concatenate all distribution data
        if all_dist_data:
            global_dist_data = np.concatenate(all_dist_data)
            global_min = np.min(global_dist_data)
            global_max = np.max(global_dist_data)
            
            if Plot_log_level >= 1:
                print(f"Global distribution range: {global_min:.3f} to {global_max:.3f}")
                print(f"Global distribution mean: {np.mean(global_dist_data):.3f}")
                print(f"Total number of elements in distribution: {len(global_dist_data)}")
        else:
            if Plot_log_level >= 1:
                print(f"Warning: No valid distribution data found in column '{dist_column}'")
            global_min = 0
            global_max = 1
    else:
        if Plot_log_level >= 1:
            print(f"Warning: Distribution column '{dist_column}' not found or is not a distribution column")
        global_min = 0
        global_max = 1
    ...
    for idx, row in df.iterrows():
        ...
        # Create figure with GridSpec for image, plot, and histogram
        fig = plt.figure(figsize=figsize, dpi=dpi)
        gs = gridspec.GridSpec(1, 3, width_ratios=[image_width_ratio, plot_width_ratio, hist_width_ratio])
        ...
        if include_A11_data and A11_y_columns_list:
        ...
        # Right subplot for histogram
        ax_hist = plt.subplot(gs[2])
        
        # Move y-axis to right side for histogram
        ax_hist.yaxis.set_label_position('right')
        ax_hist.yaxis.tick_right()
        
        # Get distribution data for the current image
        if dist_column in row and row[dist_column] is not None:
            dist_data = row[dist_column]
            if isinstance(dist_data, np.ndarray):
                # Clean distribution data (remove NaN values)
                clean_data = dist_data[~np.isnan(dist_data)]
                
                if len(clean_data) > 0:
                    # Set up bins for histogram
                    if hist_x_axis_limit is not None:
                        if isinstance(hist_x_axis_limit, (tuple, list)) and len(hist_x_axis_limit) == 2:
                            x_min, x_max = hist_x_axis_limit
                        else:
                            x_min = global_min
                            x_max = float(hist_x_axis_limit)
                    else:
                        x_min = global_min
                        x_max = global_max * hist_x_axis_extension
                    
                    # Create bins based on global min/max
                    bins = np.linspace(x_min, x_max, bin_count + 1)
                    
                    # Calculate weights for normalization if requested
                    if hist_normalize_percentage:
                        weights = np.ones_like(clean_data) * (100.0 / len(clean_data))
                    ...
                    # Plot histogram
                    hist_vals, _, patches = ax_hist.hist(
                        clean_data, bins=bins, alpha=hist_alpha, color=dist_color, 
                        label=dist_label, weights=weights, edgecolor='black', linewidth=0.5
                    )
                    
                    # Add mean line if requested
                    if show_mean_line:
                        mean_val = np.mean(clean_data)
                        # Use direct f-string with LaTeX to avoid formatting conflicts
                        # This works better than trying to use a template with both LaTeX and variable values
                        mean_line_label = f'$\\overline{{d}}_c$: {mean_val:.3f}'
                        ax_hist.axvline(
                            mean_val, color=mean_line_color, linestyle=mean_line_style,
                            linewidth=mean_line_width, 
                            label=mean_line_label
                        )
                    
                    # Set axis limits
                    if hist_y_axis_limit is not None:
                        if isinstance(hist_y_axis_limit, (tuple, list)) and len(hist_y_axis_limit) == 2:
                            y_min, y_max = hist_y_axis_limit
                        else:
                            y_min = 0
                            y_max = float(hist_y_axis_limit)
                    else:
                        y_min = 0
                        y_max = np.max(hist_vals) * hist_y_axis_extension
                    
                    ax_hist.set_ylim(y_min, y_max)
                    ax_hist.set_xlim(x_min, x_max)
                    
                    # Set labels
                    ax_hist.set_xlabel(hist_x_label, fontsize=x_label_fontsize)
                    ax_hist.set_ylabel(y_label, fontsize=x_label_fontsize)
                    
                    # Add title if provided
                    if hist_title_template:
                        ax_hist.set_title(hist_title_template.format(image_num), fontsize=x_label_fontsize)
                    
                    # Add grid if requested
                    if hist_show_grid:
                        ax_hist.grid(True, linestyle=hist_grid_style, linewidth=hist_grid_width,
                                  alpha=hist_grid_alpha, color=hist_grid_color)
                    
                    # Set tick parameters
                    ax_hist.tick_params(axis='both', direction='in', which='both', labelsize=tick_label_fontsize)
                    
                    # Double the frequency of x-axis ticks
                    current_ticks = ax_hist.get_xticks()
                    new_ticks = np.sort(np.unique(np.concatenate([
                        current_ticks,
                        (current_ticks[:-1] + current_ticks[1:]) / 2  # Add midpoints between existing ticks
                    ])))
                    ax_hist.set_xticks(new_ticks)
                    
                    # Add legend
                    if hist_legend:
                        ax_hist.legend(fontsize=legend_fontsize, frameon=False)
                else:
                    ax_hist.text(0.5, 0.5, "No valid\ndistribution data", 
                              ha='center', va='center', transform=ax_hist.transAxes)
                    ax_hist.axis('off')
            else:
                ax_hist.text(0.5, 0.5, "Distribution\ndata not\navailable", 
                          ha='center', va='center', transform=ax_hist.transAxes)
                ax_hist.axis('off')
        else:
            ax_hist.text(0.5, 0.5, "Distribution\ncolumn not\nfound", 
...
#     y_colors_list = [
#         ['darkblue'],     # First axis color
#         ['red'],      # Second axis color
#         ['green'],    # Third axis color
#     ]
    
#     # Define labels for each axis with consistent formatting
#     y_labels_list = [
#         r'$\dot{R}_{SF}/S_L$',            # First axis label
...
#     y_line_styles_list = [
#         ['-'],    # Solid line for first axis
#         ['-'],    # Solid line for second axis
#         ['-'],    # Solid line for third axis
#     ]
    
#     # Line widths for better visibility
#     y_line_widths_list = [
#         [2.0],    # Thicker line for first axis
#         [2.0],    # Thicker line for second axis
#         [2.0],    # Thicker line for third axis
#     ]
...
#     axis_spacings = [0, 85]  # Increased spacings for clearer separation
...
#     # Define A11 data parameters for plotting R_mean and R_mean_dot on different axes
#     A11_y_columns_list = [
#         ['R_mean_dot'],  # Plot R_mean_dot on axis 0
#     ]
...
#         ['-'],  # Dashed line for R_mean_dot
#     ]
    
#     A11_line_widths_list = [
#         [2.0],  # Line width for R_mean_dot
#     ]
    
#     A11_line_colors_list = [
#         ['darkblue'],  # Color for R_mean_dot
#     ]
    
#     A11_marker_styles_list = [
#         [''],  # No markers for R_mean_dot
#     ]
    
#     A11_marker_sizes_list = [
#         [6],  # Marker size for R_mean_dot
#     ]
    
#     A11_labels_list = [
#         ['A11 $\dot{R}_{SF}/S_L$'],  # Label for R_mean_dot
#     ]
    
#     A11_axis_indices_list = [
#         [0],  # Plot R_mean_dot on first axis (index 0)
#     ]
    
#     output_dir = plotter_14_xTwinyyyy_image_histogram(
#         input_dir=input_dir,
#         image_list=[],
#         omit_image_list=[106],
#         x_column="Time_VisIt",
...
#         dist_color='green',
#         dist_label=r'Cell Diameter',
#         hist_x_label=r'$d_c/\delta_T$',
#         hist_y_axis_limit=40,
#         hist_normalize_percentage=True,
#         # Output parameters
#         output_dir_comment="three_panel_enhanced_A11",
#         x_label=r'$\tau$',
#         x_label_fontsize=30,
...
#         grid_color='#CCCCCC',
#         grid_alpha=0.5,
#         image_width_ratio=0.2,
#         plot_width_ratio=0.4,
#         hist_width_ratio=0.2,
#         plot_spacing=0.3,
#         show_plot=0
#     )
    
#     print(f"Finished creating plots with enhanced A11 data plotting in: {output_dir}")
...
if __name__ == "__main__":
    ...
    # Define labels for each axis with consistent formatting
    y_labels_list = [
        r'$R_{SF}/\delta_T$',            # First axis label
        r'$N_{\mathrm{cells}}$',          # Second axis label
        r'$\overline{d}_c/\delta_T$',     # Third axis label
    ]
