...
def plotter_14_xTwinyy_1x2panel(
    ...
    # Interpolate R_mean for wrinkling factor calculation
    ...
    # Left y-axis
    line1, = ax1.plot(A11_data['R_mean_dot']['time'], A11_data['R_mean_dot']['R_mean_dot'], color='blue', label=r'$\dot{R}_{SEF}/S_L$')
    ax1.set_ylabel(r'$\dot{R}_{SEF}/S_L$', color='blue', fontsize=20)
    ...
    if legend:
        ax1.legend(lines, labels, loc='upper left', fontsize=20).get_frame().set_alpha(legend_alpha)
    ...
    line4, = ax2.plot(A11_data['A']['time'], A11_data['A']['A'], color='blue', label=r'$A_{SEF}/\delta_T^2$')
    ...
    if 'A_cell_SRec_sum_CSTx6_nonDim2' in df.columns:
        ...
        line6, = ax2_twin.plot(x_values, difference, color='black', linestyle='-', label=r'$A_{SEF}/\delta_T^2 - A_{Cells}/\delta_T^2$')
        bottom_lines.append(line6)
        ax2_twin.set_ylabel(r'$A_{SEF}/\delta_T^2 - A_{Cells}/\delta_T^2$', color='black', fontsize=20)
        ax2_twin.tick_params(axis='y', labelcolor='black', direction='out', labelsize=20)

    # Right y-axis 2 (Wrinkling factor Xi = A_SEF / (4 pi R_SEF^2))
    R_mean_interp_safe = np.where(R_mean_interp == 0, np.nan, R_mean_interp)
    wrinkling_factor = A_interp / (4 * np.pi * R_mean_interp_safe**2)
    line7, = ax2_twin2.plot(x_values, wrinkling_factor, color='purple', linestyle='--', label=r'$A_{SEF}/(4\pi R_{SEF}^2)$')
    bottom_lines.append(line7)
    ax2_twin2.set_ylabel(r'$A_{SEF}/(4\pi R_{SEF}^2)$', color='purple', fontsize=20)
    ax2_twin2.tick_params(axis='y', labelcolor='purple', direction='out', labelsize=20)

    # Bottom plot legend
    bottom_labels = [l.get_label() for l in bottom_lines]
    if legend:
        ax2.legend(bottom_lines, bottom_labels, loc='upper left', fontsize=20).get_frame().set_alpha(legend_alpha)
...
if __name__ == "__main__":
    ...
    plotter_14_xTwinyy_1x2panel(
        input_dir=input_dir,
        output_dir_comment="Area_Difference_and_WrinklingFactor PROCI",
        show_plot=0
    )
