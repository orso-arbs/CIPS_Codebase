...
def Visit_projector_1(
    ...
    # VisIt and cluster parameters
    MashineProfileName = "euler.ethz.ch", # Name of the machine profile to use
    cluster_args_userName = "orsob", # Username for the cluster
    cluster_args_numProcessors = 4, # Number of processors to use
    cluster_args_numNodes = 1, # Number of nodes to use
    cluster_args_timeLimit = "05:00:00",
    ...
    Pseudocolor_colortable = "PointWise", # Can be "hot", "Cu stomBW1", "PeriodicBW", "PointWise", etc.
    ...
    WindowWidth = 2000, WindowHeight = 2000, # Window size
    ...
    VisIt_sitepackages_path : str, optional
        Path to the VisIt Python module site-packages directory. This is used to import
        the VisIt Python module. The path depends on your VisIt installation location.
        Default is "C:\\Users\\obs\\LLNL\\VisIt3.4.2\\lib\\site-packages".
    cluster_args_additional : str, optional
        Additional arguments to pass to the cluster job submission command when launching
        VisIt in parallel mode. These typically include memory requirements, output file paths,
        and any other cluster-specific parameters needed for job allocation.
        Default provides 6GB memory per CPU, 6GB temp space, and redirects logs to specific paths.
    ...
    if not _VISIT_INITIALIZED:
        ...
        sys.path.append(VisIt_sitepackages_path)
        import visit as vi
    ...
    p = vi.GetMachineProfile(MashineProfileName)
    #print(p) # uncomment to see the machine profile
    p.userName = cluster_args_userName
    ...
    launch_profile.numProcessors = cluster_args_numProcessors
    launch_profile.numNodes = cluster_args_numNodes
    launch_profile.timeLimit = cluster_args_timeLimit
    launch_profile.launchArgs = cluster_args_additional # Set launchArgs on the retrieved profile object
    ...
    if "Pseudocolor - Isosurface" in Plots:
        ...
        if Pseudocolor_colortable == "CustomBW1":
            ...
            # Create a subdirectory for color tables
            colortable_storage_dir = os.path.join(output_dir, "colortables")
            create_pointwise_bw_color_table(
...
if __name__ == "__main__":
    ...
    # View parameters
    no_annotations = 1, viewNormal = [0,-1,0], viewUp = [0,0,1], imageZoom = 1, parallelScale = 120, perspective = 0, # View
    WindowWidth = 2000, WindowHeight = 2000, # Window size
    Visit_projector_1_show_windows = 0, # Window

    # output and logging
    Visit_projector_1_log_level = 5,
    output_dir_manual = "", output_dir_comment = "",
