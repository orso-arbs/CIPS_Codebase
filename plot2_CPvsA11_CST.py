...
def plotter_2_CPvsA11_CST(input_dir, # Format_1 requires input_dir
    ...
    CP_data_df['A_cell_CST_sum_nonDim2'] = CP_data_df['A_cell_distribution_CST_nonDim2'].apply(
        lambda x: np.sum(x) if x.size > 0 else 0)
    ...
    bin_size = max_diameter/15 if max_diameter > 0 else 0.1
    max_frequency = 0
    ...
    for i in range(N_images): # Plot the data for each row
        ...
        ax_0_2.set_title(f"Image {i+1} with Masks (CST cells: {CP_data_df.loc[i, 'N_cells_CST']})")
        ax_0_2.axis('off')
        ...
        ax_1_0.set_title("CST Diameter Distribution")
        ...
        if has_cst_data and pd.notna(CP_data_df.loc[i, 'd_cell_CST_mean_nonDim']):
            ...
            ax_1_0.text(ax_1_0.get_xlim()[1] * 0.5, mean_diameter + max_diam_limit*0.01, 
                      f'CST Mean: {mean_diameter:05.2f}', color='green')
        
        if pd.notna(CP_data_df.iloc[i]['diameter_training_nonDim']): # make sure diameter training is available
            ...
            ax_1_0.text(ax_1_0.get_xlim()[1] * 0.6, diameter_training_nonDim + max_diam_limit*0.01, 
                      f"Training: {diameter_training_nonDim:05.2f}", color='violet')
        ...
        if 'd_cell_CST_mean_nonDim' in CP_data_df.columns:
            line_1_12_1, = ax_1_12.plot(CP_data_df['time'], CP_data_df['d_cell_CST_mean_nonDim'], 
                                     ...
                                     " = CST Cell Mean Diameter $D_{c,mean}$", color='green')
        else:
        ...
        line_1_12_2, = ax_1_12.plot(CP_data_df['time'], CP_data_df['diameter_training_nonDim'], 
                                 label=f"{CP_data_df.iloc[i]['diameter_training_nonDim'] if pd.notna(CP_data_df.iloc[i]['diameter_training_nonDim']) else 0.00:05.2f} = Cellpose Training Diameter", 
                                 color='aquamarine')
        ...
        S2 = 1e-1
        line_1_12_3, = ax_1_12.plot(A11_SF_R_mean['time'], A11_SF_R_mean['R_mean'] * S2, 
                                  label=f"{(closest_A11_r_mean*S2):05.2f} = A11 Spherical Flame Radius * {S2:.3f}", 
                                  color='olive', linestyle='dashed')

        S3 = 1
        ax_1_12_L = ax_1_12.twinx()
        line_1_12_L, = ax_1_12_L.plot(A11_SF_iHRR['time'], A11_SF_iHRR['iHRR'] * S3, 
                                     label=f"{(closest_A11_iHRR*S2):05.2f} = A11 integral heat release rate * {S2:.3f}", 
                                     color='orange', linestyle='dashed')

        # Add CST cell count
        ax_1_12_R = ax_1_12.twinx()
        line_1_12_R, = ax_1_12_R.plot(CP_data_df['time'], CP_data_df['N_cells_CST'], 
                                    label=f"{CP_data_df.iloc[i]['N_cells_CST']:05.2f} = Number of CST cells", 
                                    color='red')

        # Mark current image time
        line_1_12_4 = ax_1_12.axvline(CP_data_df.iloc[i]['time'], color='black', 
                                    label=f"{CP_data_df.iloc[i]['image_number']:.2f} = shown image", 
        ...
        ax_1_12_R.set_ylabel("Number of CST Cells", color='red')
        ...
        # Add a title indicating this is CST data
        plt.suptitle("CST-Filtered Cell Analysis", fontsize=16, y=0.98)

        # Adjust layout and save the figure as a PNG file
        plt.tight_layout(rect=[0, 0, 1, 0.95])  # Make room for the suptitle
        plot_filename = os.path.join(output_dir, f'plot_CST_{i+1:04d}.png')
    ...
    if video == 1:
        vm1.create_video_from_images(
            ...
            fps=5,
            video_name="CST_analysis_video.mp4"
            )

    #################################################### return
    
    print(f"Plotter 2 (CPvsA11_CST) finished. Output in {output_dir}")
    return output_dir # Format_1 requires output_dir as first return

# Example usage when script is run directly
if __name__ == "__main__":
    print("Running plotter_2_CPvsA11_CST as standalone module...")
    ...
    output_dir = plotter_2_CPvsA11_CST(
        input_dir=input_dir,
        video=1,
        Plot_log_level=2,
        output_dir_comment="CST_selected_test"
    )
